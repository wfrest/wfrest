...
namespace wfrest
{
    ...
    public:
        ...
        std::string full_path() const { return route_full_path_; }
        void set_full_path(const std::string& route_full_path) { route_full_path_ = route_full_path; }

        void set_parsed_uri(ParsedURI&& parsed_uri) { parsed_uri_ = std::move(parsed_uri); }
        void set_query_params(QueryParams&& query_params) { query_params_ = std::move(query_params); }
        std::string query(const std::string& key) { return query_params_[key]; }
        std::string default_query(const std::string& key, const std::string& default_val);
        QueryParams query_list() const { return query_params_; }
    private:
