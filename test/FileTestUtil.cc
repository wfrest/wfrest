...
bool FileTestUtil::write_file(const std::string &file_name, const std::string& body)
{
...
int FileTestUtil::create_dir(const char *path, mode_t mode)
{
    struct stat st;
    int status = 0;
    if(stat(path, &st) != 0)
...
int FileTestUtil::recursive_delete(const char *dir)
{
    int ret = 0;
    FTS *ftsp = NULL;
    FTSENT *curr;
